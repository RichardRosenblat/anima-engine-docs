...
### 2.1 Synapse (Transport Abstraction)

The **Synapse** is the transport abstraction layer that implements the mTLS connection requirements defined above.

A Synapse:

* Represents a connection to an out-of-process module
* Attaches lease proofs and execution metadata to all invocations
* Handles serialization and transport-level concerns
* Provides an opaque invocation surface to higher layers
* Enforces metadata requirements automatically
* Abstracts the transport mechanism from business logic
* Makes process boundaries explicit

Synapses contain **no business semantics or authorization logic** — they delegate to the lease authority and exist purely to enforce process boundary separation and ensure lease binding for all invocations.
