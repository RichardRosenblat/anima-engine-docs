...
### 2.1 Synapse (Abstração de Transporte)

O **Synapse** é a camada de abstração de transporte que implementa os requisitos de conexão mTLS definidos acima.

Um Synapse:

* Representa uma conexão a um módulo fora de processo
* Anexa provas de lease e metadados de execução a todas as invocações
* Lida com serialização e preocupações de nível de transporte
* Fornece uma superfície de invocação opaca para camadas superiores
* Aplica requisitos de metadados automaticamente
* Abstrai o mecanismo de transporte da lógica de negócio
* Torna limites de processo explícitos

Synapses não contêm **semântica de negócio ou lógica de autorização** — eles delegam à autoridade de lease e existem puramente para aplicar separação de limite de processo e garantir vinculação de lease para todas as invocações.
...
* Lista negra
* Reimplantação obrigatória ou isolamento em sandbox
